...
ll get_indices(double h, double *F, int N) {
    int i, j;
    for (i = 0; i < N; i++)
        for (int j = i; j < N; j++)
            if (F[i] + F[j] == h) {
                printf("%f: found %d %d => %lld\n", h, i, j, (ll) i * i + (ll) j * j);
...
void solve(int n) {
    ...
    if (!F) error("Memory overflow");
    if (!two) error("Memory overflow");
    ...
    printf("g = %lld\n", get_indices(b1, F, N) + get_indices(b2, F, N));
