...
class FactorisationSieve():
	...
	def __sieve_n2_plus_one(self, bound):
		...
		self.factorisation = [{} for n in range(bound + 1)]
...
class Problem():
	...
	def get_mod(self, bound, mod):
		self.sieve = FactorisationSieve(bound + 1)
		result = 0
	...
	def __get_retraction_count(self, n):
		...
		z = self.__merge_factorization(x, y)
		result = 1
		for d in z:
			result *= (d**z[d] + 1)
	...
	def __merge_factorization(self, x, y):
		result = dict(x)
		for d in y:
			if d not in result:
				result[d] = 0
